         ...
         for (int i = 0; i < PlayerStats.missionList.Count; i++)
        {
            if (PlayerStats.missionList[i] != null)
            {
                if (PlayerStats.missionList[i].GetQuestObjective().goalType == GoalType.ReportTo && PlayerStats.missionList[i].GetStatus() == true)
                {
                    ...
                    Debug.Log("THIS HERE OFFICER: " + PlayerStats.missionList[i].GetQuestObjective().reporterName);
                    Debug.Log("THIS HERE OFFICER 2: " + finisher.gameObject.name);
                    if (PlayerStats.missionList[i].GetQuestObjective().reporterName.Equals(finisher.gameObject.name))
                    {
